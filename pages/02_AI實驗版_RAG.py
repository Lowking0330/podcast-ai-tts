...
with st.sidebar:
    ...
    **5. 🤖 AI 寫劇本** (新!) 讀資料自動寫對話。
    ...
    # AI Key 設定區
    st.markdown("---")
    st.markdown("#### 🔑 AI 設定 (RAG)")
    if 'gemini_key' not in st.session_state:
        st.session_state['gemini_key'] = ''
    
    user_key = st.text_input("Google Gemini API Key", value=st.session_state['gemini_key'], type="password", help="請至 Google AI Studio 申請免費 Key")
    if user_key:
    ...
    st.markdown("---")
    st.success("✅ 系統狀態：正常")
    st.caption("版本: Podcast-015 | 核心: Free Tier")

st.title("🎙️ 族語Podcast內容產製程式")
st.markdown("打造您的專屬原住民族語廣播節目，支援 **16族42語**、**雙語教學** 與 **AI 自動生成**。")
            ...
            try:
                ...
                for idx, item in enumerate(dialogue):
                    ...
                    if zh:
                        ...
                        success, _ = generate_chinese_audio_free_tier(zh, zh_gender, tmp_zh_path)
...
# ==========================================
with tab5:
    st.markdown("### 🤖 AI 自動寫劇本 (RAG)")
    st.caption("上傳 PDF 或貼上文字，AI 自動幫您寫成雙人廣播劇腳本。")
    
    # 檢查 API Key
    api_key = st.session_state.get('gemini_key', '')
    if not api_key:
        st.warning("⚠️ 請先在左側側邊欄輸入 Google Gemini API Key")
    ...
    with st.container(border=True):
        ...
        if input_method == "貼上文字":
            context_text = st.text_area("請貼上參考資料", height=200, placeholder="貼上部落故事、新聞或文化介紹...")
        else:
        ...
        with c_ai2:
            role_a = st.text_input("角色 A 名稱", value="老師")
    ...
    if st.button("🚀 AI 生成劇本", type="primary", disabled=not api_key, use_container_width=True):
        if not context_text:
            ...
            try:
                with st.spinner("AI 正在閱讀資料並撰寫劇本中... (約需 10-20 秒)"):
                    # 呼叫 Gemini
                    ...
                    st.success(f"成功生成 {len(script_data)} 句對話！")
                    st.info("💡 劇本已自動載入。請切換到 **「🏫 Podcast II (雙語教學)」** 分頁進行合成。")
                    
                    # 預覽結果
                    with st.expander("查看生成的劇本內容", expanded=True):
                        st.json(script_data)
